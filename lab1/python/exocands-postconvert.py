...
names = np.genfromtxt('../FINAL_LIST.csv', delimiter=',', usecols=[3], skip_header=1, dtype=str)
other_data = np.genfromtxt('../FINAL_LIST.csv', delimiter=',', usecols=[0, 2, 8], skip_header=1)
...
for n in range(len(names)):
    ...
    for i in range(len(az)):
        if alt[i] >= 40:
            print('{0:6.3f}  {1:5.3f}  {2}  {3:4.4f}  {4:3.2f}  {5:4.3f}'.format(
